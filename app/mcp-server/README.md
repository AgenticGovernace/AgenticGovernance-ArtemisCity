<p><a target="_blank" href="https://app.eraser.io/workspace/OSoW4GLh52O0xu6Y8Mm3" id="edit-in-eraser-github-link"><img alt="Edit in Eraser" src="https://firebasestorage.googleapis.com/v0/b/second-petal-295822.appspot.com/o/images%2Fgithub%2FOpen%20in%20Eraser.svg?alt=media&amp;token=968381c8-a7e7-472a-8ed6-4a6626da5501"></a></p>
...
1. **Build and run the Docker container:**This command will build the Docker image (if it doesn't exist or has changed) and start the MCP server in a container.docker-compose up --build
2. **Verify:**
...
- `POST /getContext` 
    ...
    - **Request Body:**{ "path": "path/to/your/Note.md" }
    - **Example (curl):**curl -X POST http://localhost:3000/api/getContext \
...
-d '{ "path": "My Daily Notes/2023-10-27.md" }'
  ...
  "data": {
    ...
    - **Error Response (400 Bad Request / 500 Internal Server Error):**{ "success": false, "error": "Missing note path." }{ "success": false, "error": "Error reading note '...' : ..." }

---

- `POST /appendContext` 
    - **Description:** Appends content to an existing note or creates a new one if it doesn't exist.
    - **Request Body:**{ "path": "path/to/your/Note.md", "content": "New content to append." }
    - **Example (curl):**curl -X POST http://localhost:3000/api/appendContext \
-H "Content-Type: application/json" \
-H "Authorization: Bearer <YOUR_MCP_API_KEY>" \
...
- `POST /updateNote` 
    ...
    - **Request Body:**{ "path": "path/to/your/Note.md", "content": "New full content." }
    - **Example (curl):**curl -X POST http://localhost:3000/api/updateNote \
-H "Content-Type: application/json" \
-H "Authorization: Bearer <YOUR_MCP_API_KEY>" \
-d '{ "path": "My Project/Status.md", "content": "# Project Status\n\nUpdated: 2023-10-27\n\nAll tasks are green." }'
    - **Success Response (200 OK):**{ "success": true, "message": "Note 'My Project/Status.md' updated." }
...
- `POST /searchNotes` 
    ...
    - **Example (curl):**curl -X POST http://localhost:3000/api/searchNotes \
-H "Content-Type: application/json" \
-H "Authorization: Bearer <YOUR_MCP_API_KEY>" \
...
- `POST /listNotes` 
    ...
    - **Example (curl):**curl -X POST http://localhost:3000/api/listNotes \
-H "Content-Type: application/json" \
-H "Authorization: Bearer <YOUR_MCP_API_KEY>" \
-d '{}'
    - **Success Response (200 OK):**{
...
- `POST /deleteNote` 
    ...
    - **Request Body:**{ "path": "path/to/note/to/delete.md" }
    - **Example (curl):**curl -X POST http://localhost:3000/api/deleteNote \
-H "Content-Type: application/json" \
-H "Authorization: Bearer <YOUR_MCP_API_KEY>" \
...
- `POST /manageFrontmatter` 
    ...
    - **Request Body:**{ "path": "path/to/note.md", "key": "status", "value": "completed" }
    - **Example (curl):**curl -X POST http://localhost:3000/api/manageFrontmatter \
-H "Content-Type: application/json" \
-H "Authorization: Bearer <YOUR_MCP_API_KEY>" \
...
- `POST /manageTags` 
    ...
    - **Request Body (Add Tags):**{ "path": "path/to/note.md", "tags": ["tag1", "tag2"], "action": "add" }
    - **Request Body (Remove Tags):**{ "path": "path/to/note.md", "tags": ["tag1"], "action": "remove" }
    - **Example (curl - Add):**curl -X POST http://localhost:3000/api/manageTags \
-H "Content-Type: application/json" \
-H "Authorization: Bearer <YOUR_MCP_API_KEY>" \
-d '{ "path": "My Project/Task A.md", "tags": ["urgent", "review"], "action": "add" }'
    - **Example (curl - Remove):**curl -X POST http://localhost:3000/api/manageTags \
-H "Content-Type: application/json" \
-H "Authorization: Bearer <YOUR_MCP_API_KEY>" \
...
- `POST /searchReplace` 
  ...
  "replace": "new string"
    ...
    - **Example (curl):**curl -X POST http://localhost:3000/api/searchReplace \
-H "Content-Type: application/json" \
-H "Authorization: Bearer <YOUR_MCP_API_KEY>" \
...
<!--- Eraser file: https://app.eraser.io/workspace/OSoW4GLh52O0xu6Y8Mm3 --->
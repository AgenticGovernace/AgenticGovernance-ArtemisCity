...
class _Approx:
    def __init__(self, e, rel=1e-6, abs=1e-9): self.e, self.rel, self.abs = e, rel, abs
    def __eq__(self, a):
        try: return abs(a - self.e) <= max(self.abs, self.rel * abs(self.e))
...
for modname, path in targets:
    ...
    for fname, fn in sorted(inspect.getmembers(m, inspect.isfunction)):
        if not fname.startswith("test_"): continue
        total += 1
        kwargs = {}
        if "tmp_path" in inspect.signature(fn).parameters:
            kwargs["tmp_path"] = Path(tempfile.mkdtemp())
        try:
            fn(**kwargs); print(f"PASS  {fname}")
        except Exception:
            fail += 1; print(f"FAIL  {fname}"); traceback.print_exc()

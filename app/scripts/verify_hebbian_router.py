...
for f in ["src/mcp/orchestrator.py", "src/integration/hebbian_router.py", "src/tests/test_hebbian_router.py"]:
    try:
        py_compile.compile(f"{REPO}/{f}", doraise=True); print("COMPILE OK ", f)
...
for name, fn in sorted(inspect.getmembers(m, inspect.isfunction)):
    ...
    try:
        ...
        fail += 1; print("FAIL ", name); traceback.print_exc()

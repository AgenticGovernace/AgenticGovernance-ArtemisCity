...
class LLMController {
     ...
     */
    async streamChat(messages, model, options = {}, onChunk = () => { }) {
        const selectedModel = model || this.defaultModel;
        const id = `stream-${Date.now()}`;
        // Simulate streaming
        const fullResponse = await this.simulateChat(messages, selectedModel, options);
        const words = fullResponse.content.split(' ');
        const MAX_STREAM_WORDS = 2000;
        const maxWords = Math.min(words.length, MAX_STREAM_WORDS);
        for (let i = 0; i < maxWords; i++) {
            await new Promise(resolve => setTimeout(resolve, 50));
            onChunk({
                id,
                delta: words[i] + (i < maxWords - 1 ? ' ' : ''),
                finishReason: i === maxWords - 1 ? 'stop' : undefined
            });
        }
    }
    /**
     ...
     */
    async processATP(atpMessage, model, agentId) {
    ...
    async simulateChat(messages, model, options) {
    ...
    buildATPSystemPrompt(atpMessage, agentId) {
...
Respond appropriately for the given context and priority level.`;
    ...
    getMaxTokensForActionType(actionType) {
        const maxTokens = {
            'CREATE': 2048,
            'READ': 1024,
            'UPDATE': 1024,
    ...
    logUsage(provider, model, promptTokens, completionTokens) {
        usageLog.push({

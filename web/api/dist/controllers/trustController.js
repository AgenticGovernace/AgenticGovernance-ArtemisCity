...
const hebbianWeights = new Map();
// Initialize default trust scores for known agents
const defaultTrustScores = [
    {
        entityId: 'artemis',
        entityType: 'agent',
        score: 0.95,
...
class TrustController {
     ...
     */
    async setTrustScore(entityId, score, entityType = 'agent') {
     ...
     */
    async recordSuccess(entityId, amount = 0.02) {
     ...
     */
    async recordFailure(entityId, amount = 0.05) {
        const trust = trustStore.get(entityId);
        if (!trust)
            return null;
     ...
     */
    async canPerformOperation(entityId, operation) {
        const trust = trustStore.get(entityId);
        if (!trust)
            return false;
    ...
    // Helper methods
    scoreToLevel(score) {
        if (score >= TRUST_THRESHOLDS.FULL)
            return 'FULL';
...
//# sourceMappingURL=trustController.js.map
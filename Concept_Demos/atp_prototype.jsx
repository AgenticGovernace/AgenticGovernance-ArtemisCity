...
const AGENTS = {
  ...
  planner: {
    name: 'Planner',
    role: 'Architect, Planning and Contextual Assistance',
    icon: Brain,
    color: '#ff6b6b',
...
const GlowCard = ({ children, className = '', glowColor = 'cyan' }) => {
  ...
  return (
    <div
      className={`bg-slate-900/80 backdrop-blur-xl border rounded-lg shadow-lg transition-all duration-300 ${glowClasses[glowColor]} ${className}`}
    >
      {children}
    </div>
  );
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">
            Compose structured commands for the Artemis system
          </p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="p-4 rounded-lg bg-slate-950 border border-slate-700/50 font-mono text-sm min-h-[120px]">
              ...
              <span className="text-white">{message.mode || '___'}</span>{' '}
              <span className="text-green-400">#Context:</span>{' '}
              <span className="text-white">{message.context || '___'}</span>{' '}
              <span className="text-amber-400">#Priority:</span>{' '}
              <span className="text-white">{message.priority || 'Normal'}</span>{' '}
              <span className="text-purple-400">#ActionType:</span>{' '}
              <span className="text-white">{message.actionType || 'Execute'}</span>
              {message.targetZone && (
                <>
                  <br />
                  <span className="text-pink-400">#TargetZone:</span>{' '}
                  <span className="text-white">{message.targetZone}</span>
                </>
              )}
            </div>
          </div>
          ...
          return (
            <div
              ...
              className={`p-4 rounded-lg border transition-all duration-500 ${
                isActive ? 'border-2 scale-105 shadow-lg' : 'border-slate-700/50 bg-slate-800/30'
              }`}
              style={{
            ...
            >
              ...
              <div className="flex flex-wrap gap-1">
                {agent.keywords.slice(0, 3).map((kw) => (
                  <span
                    key={kw}
                    className="px-1.5 py-0.5 rounded text-[10px] bg-slate-700/50 text-slate-400 font-mono"
                  >
                    {kw}
                  </span>
                ))}
      ...
      {scenarios[scenario].events.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {scenarios[scenario].events.map((event, i) => (
            <div
              key={i}
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800/50 text-xs font-mono"
            >
...
→ Matched: Copilot (Elastic Augmentation)`,
  ...
  return (
    <GlowCard className="p-6" glowColor="amber">
      ...
      {/* Current Step */}
      <div
        className={`p-6 rounded-lg border-2 transition-all duration-500 ${highlightColors[steps[step].highlight]}`}
      >
...
const Hero = () => (
  <div className="text-center mb-12">
    ...
    <p className="text-lg text-slate-400 max-w-2xl mx-auto">
      Structured communication for agentic systems. Balance trust, entropy, and collaboration to
      achieve <span className="text-cyan-400 font-semibold">net good over noise</span>.
    </p>
  </div>
);
    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="space-y-6">
          ...
          {activeTab === 'routing' && <AgentRouting message={message} />}

          {activeTab === 'trust' && <TrustDecay />}

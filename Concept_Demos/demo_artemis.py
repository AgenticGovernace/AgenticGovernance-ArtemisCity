...
def _warn_if_shadowed(package_name: str) -> None:
    """Best-effort warning if a package is being imported from an unexpected location."""
    try:
    ...
    # If the imported package isn't inside this repo, warn.
    if repo_root not in pkg_path.parents:
        print(
            f"Warning: '{package_name}' is being imported from outside this repo:\n"
            f"  {pkg_path}\n"
            f"This can cause import errors or unexpected behavior.\n"
        )

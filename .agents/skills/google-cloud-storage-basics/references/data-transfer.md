...
1. **Cross-Cloud Migration (AWS S3, Azure Blob Storage):**
   - _Recommendation:_ Always use **Storage Transfer Service (STS)**. It runs
...
2. **On-Premises Migration:**
   - _Recommendation:_ If the dataset is **under 1TB**, it is generally
...
3. **Few Large Files (size >= 150MB, count < 10K):**
   - _Recommendation:_ Use `gcloud storage cp`. By default, it enables
...
hard-deleting the temporary components _during_ composition instead of deleting

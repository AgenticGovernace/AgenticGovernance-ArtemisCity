...
If you use the typed `McpServer` wrapper, pass it in directly — the SDK will unwrap it and also install a proxy on `_registeredTools`, so any tool you register _after_ `instrument()` is also wrapped:
...
Minting only reaches the wire in JSON mode. In SSE (streaming) mode, `StreamableHTTPServerTransport` builds the response headers _before_ your `initialize` handler runs, so the minted header never lands and the SDK silently falls back to a session per request:

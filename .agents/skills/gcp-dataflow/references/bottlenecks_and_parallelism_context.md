...
- **Explicit vs. Implicit Keys**:
  - _Explicit_: User-defined keys resulting from pipeline design (e.g.,
    grouped-by keys).
  - _Implicit_: Assigned automatically by the system when no semantic key
...
- **Serial Processing**: Message processing is strictly **serialized per key**
  ...
  - _Mechanism_: While a batch of messages is processing for active key $K$,
    subsequent messages for $K$ are buffered.
  - _Rationale_: Permits highly efficient state caching and "blind writes",
    avoiding expensive transactions or fine-grained synchronization
    barriers.
  - _Minimum State_: Under the hood, even purely stateless fused stages
...
- **Bottlenecks & Amdahl's Law**:
  - _Low Key Cardinality_: Insufficient key variety constrains parallel
    execution, leading to idle workers and slow processing.
  - _Hot Keys_: Uneven distribution of traffic where a tiny subset of keys
    ...
    age, backlog).
  - _High Key Cardinality_: Extremely high cardinality of keys together with

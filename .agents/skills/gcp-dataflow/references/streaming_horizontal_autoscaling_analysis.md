...
- **Proactive Scale Up (High CPU)**: If CPU utilization spikes aggressively,
  the autoscaler proactively upscales _before_ a backlog develops, absorbing
...
- **Worker Utilization Hint (`job/worker_utilization_hint`)**:
  ...
  - _Lower hint_: Prevents aggressive worker termination, retaining warm
    capacity.
  - _Higher hint_: Maximizes worker density but requires highly parallelized

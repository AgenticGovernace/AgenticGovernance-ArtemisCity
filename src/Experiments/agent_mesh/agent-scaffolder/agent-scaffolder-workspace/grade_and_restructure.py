...
ITER = Path("/sessions/busy-vibrant-tesla/mnt/outputs/agent-scaffolder-workspace/iteration-1")

# timing captured from the subagent task notifications
TIMING = {
    ("refactorbot-folder-scaffold", "with_skill"):    {"total_tokens": 31582, "duration_ms": 75705},
    ("refactorbot-folder-scaffold", "without_skill"): {"total_tokens": 36026, "duration_ms": 159327},
    ("finlit-system-prompt", "with_skill"):           {"total_tokens": 33844, "duration_ms": 90945},
    ("finlit-system-prompt", "without_skill"):        {"total_tokens": 25531, "duration_ms": 77394},
    ("multiagent-agents-md-atp", "with_skill"):       {"total_tokens": 33231, "duration_ms": 93609},
    ("multiagent-agents-md-atp", "without_skill"):    {"total_tokens": 34477, "duration_ms": 135810},
}
...
def grade(eval_id, paths, t):
    exp = []
    if eval_id == 0:
        exp.append(E("Creates an AGENTS.md file", has_file(paths, "AGENTS.md"),
                     f"files: {paths}"))
        exp.append(E("Creates an index.md folder-context file", has_file(paths, "index.md"),
                     f"files: {paths}"))
        exp.append(E("Creates a .codex/instructions.md behavior file",
                     has_path(paths, ".codex") and has_file(paths, "instructions.md"),
                     f"files: {paths}"))
        layers = all(k in t for k in ["role", "mission", "output standard", "escalation"])
        exp.append(E("Agent Card uses the Role / Mission / Output Standards / Escalation layers",
                     layers, "searched concatenated output text for the four layer labels"))
        diffs = "diff" in t and ("raw" in t or "only" in t or "unified" in t)
        exp.append(E("Encodes the raw-diffs-only output rule", diffs,
                     "looked for 'diff' plus 'raw'/'only'/'unified'"))
        tone = "casual" in t or "formal" in t
        exp.append(E("Encodes the non-casual / formal tone rule", tone,
                     "looked for 'casual' or 'formal'"))
        perf = "performance" in t and bool(re.search(r"per[ -](line|hunk|change)", t))
        exp.append(E("Encodes the per-line performance-gain rule", perf,
                     "looked for 'performance' plus 'per line/hunk/change'"))
    elif eval_id == 1:
        exp.append(E("Produces a system-prompt / Agent Card markdown file",
                     any(p.lower().endswith(".md") for p in paths), f"files: {paths}"))
        exp.append(E("Includes a Role layer", "role" in t, "searched output text"))
        exp.append(E("Includes a Mission layer", "mission" in t, "searched output text"))
        exp.append(E("Includes an Output Standards layer", "output standard" in t,
                     "searched output text"))
        exp.append(E("Includes an Escalation Rules layer", "escalation" in t,
                     "searched output text"))
        invest = ("invest" in t) and bool(re.search(r"\b(not|no|never|avoid|don't|cannot|can't|must not|won't|refrain|prohibit)\b", t))
        exp.append(E("Explicitly prohibits specific investment buy/sell advice", invest,
                     "looked for 'invest' plus a negation token"))
        friendly = any(w in t for w in ["friendly", "warm", "encouraging", "supportive"])
        exp.append(E("Specifies a friendly / warm tone", friendly,
                     "looked for friendly/warm/encouraging/supportive"))
    elif eval_id == 2:
        exp.append(E("Produces an AGENTS.md file", has_file(paths, "AGENTS.md"), f"files: {paths}"))
        exp.append(E("Defines a Writer agent", "writer" in t, "searched output text"))
        exp.append(E("Defines a Reviewer agent", "reviewer" in t, "searched output text"))
        exp.append(E("Uses the Agent Card formula (Role + Mission)",
                     "role" in t and "mission" in t, "searched output text"))
        atp = ("atp" in t) or ("artemis" in t) or ("transmission protocol" in t)
        exp.append(E("References the ATP / artemis-transmission-protocol communication layer",
                     atp, "looked for 'atp'/'artemis'/'transmission protocol'"))
    return exp
...
for ev in EVALS:
    for config in ["with_skill", "without_skill"]:
        ...
        timing = {"total_tokens": tm["total_tokens"], "duration_ms": tm["duration_ms"],

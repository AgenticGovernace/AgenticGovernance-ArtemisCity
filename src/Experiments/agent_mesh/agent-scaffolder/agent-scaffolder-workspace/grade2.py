...
FRESH = {
    ...
    ("audit-agent-provenance", "without_skill"): {
        "total_tokens": 80749,
        "duration_ms": 463784,
    },
}
...
def has_file(paths, b):
    return any(Path(p).name.lower() == b.lower() for p in paths)


def has_path(paths, n):
    return any(n.lower() in p.lower() for p in paths)
    ...
    if eid == 0:
        ...
        e.append(
            E(
                ...
                "looked for diff + raw/only/unified",
            )
        )
        e.append(
            E(
        ...
        e.append(
            E(
                ...
                "performance + per line/hunk/change",
            )
        )
    elif eid == 1:
        e.append(
        ...
        e.append(
            E(
                ...
                has_file(paths, "AGENTS.md") or ("agent card" in t),
                f"files: {paths}",
            )
        )
        e.append(
            E(
                "Encodes observe-and-log-only boundaries (never modifies watched files)",
                bool(
                    re.search(
                ...
                or "read-only" in t
                or "read only" in t,
                "observe/log-only or no-modify rule",
            )
        )
        e.append(
            E(
        ...
        e.append(
            E(
                ...
                "reflect/summary + cadence token",
            )
        )
        e.append(
            E(
                "References the atp-provenance-logging skill (parent/child prov_ids in agent_logs)",
                ("atp-provenance-logging" in t)
                or ("agent_logs" in t)
                or ("parent_prov_id" in t),
                "skill-specific provenance terms",
            )
        )
    return e
...
def write_run(eid, name, config, src_outputs, timing_data):
    ...
    timing = {
        "total_tokens": timing_data["total_tokens"],
        "duration_ms": timing_data["duration_ms"],
        "total_duration_seconds": round(timing_data["duration_ms"] / 1000, 1),
    }
    meta = {
        "eval_id": eid,
        "eval_name": name,
        "prompt": next(e["prompt"] for e in EVALS if e["id"] == eid),

...
obsidianAPI.interceptors.request.use(
  (config) => {
    logger_1.logger.debug(
      `Obsidian API Request: ${config.method?.toUpperCase()} ${config.url}`,
    );
    return config;
  },
  (error) => {
...
obsidianAPI.interceptors.response.use(
  (response) => {
    logger_1.logger.debug(
      `Obsidian API Response: ${response.status} ${response.config.url}`,
    );
    return response;
  },
  (error) => {
    if (error.response) {
      logger_1.logger.error(
        `Obsidian API Response Error - Status: ${error.response.status}, Data: ${JSON.stringify(error.response.data)}`,
      );
    } else if (error.request) {
      logger_1.logger.error(
        `Obsidian API Response Error - No response received: ${error.message}`,
      );
    } else {
      logger_1.logger.error(
        `Obsidian API Request Setup Error: ${error.message}`,
      );
    }
    return Promise.reject(error);
  },
